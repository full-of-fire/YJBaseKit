...
- (void)yj_pushViewController:(UIViewController *)viewController
{
    @autoreleasepool
    {
        ...
        [[self yj_currentNaviController] pushViewController:viewController animated:YES];
    }
}

- (UIViewController *)yj_popViewController
{
    return [[self yj_currentNaviController] popViewControllerAnimated:YES];
}

- (NSArray *)yj_popToRootViewController
{
    return [[self yj_currentNaviController] popToRootViewControllerAnimated:NO];
}

- (NSArray *)yj_popToViewController:(UIViewController *)viewController
{
    return [[self yj_currentNaviController] popToViewController:viewController animated:YES];
}
...
- (void)yj_postNotificationWithName:(NSString *)name userInfo:(NSDictionary *)userInfo{
...
- (void)_xw_swizzleDealloc{
    ...
    @synchronized(swizzleClass) {
        ...
        id newDealloc = ^(__unsafe_unretained id objSelf){
            //在这里我们移除所有的KVO
            [objSelf yj_removeAllObserverBlocks];
            //移除所有通知
            [objSelf yj_removeAllNotification];
